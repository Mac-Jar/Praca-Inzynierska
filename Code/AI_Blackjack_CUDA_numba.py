...
@cuda.jit(device=True, inline=True)
def draw_card_xor(rng_states, tid):
...
def play_episodes_kernel(dev_Q, epsilon, rng_states,
    ...
    for ep in range(episodes_per_thread):
        ...
        # draw cards (mapping)
        v = draw_card_xor(rng_states, tid); pcards[p_n] = map_card_value(v); p_n += 1
        v = draw_card_xor(rng_states, tid); pcards[p_n] = map_card_value(v); p_n += 1
        v = draw_card_xor(rng_states, tid); dcards[d_n] = map_card_value(v); d_n += 1
        v = draw_card_xor(rng_states, tid); dcards[d_n] = map_card_value(v); d_n += 1
        ...
        while True:
            ...
            if a == 1:
                # hit
                v = draw_card_xor(rng_states, tid)
